...
-   **Insects** (`🦋`, `🐛`, `🐌`, `🐞`, `🐝`):
    -   **AI & Movement**: Insects move with purpose, but not with perfect uniformity. They use a Quadtree to find the nearest flower within their vision range and move towards it. To prevent them from clumping and moving in unnatural swarms, a small amount of randomness is introduced into their pathfinding. Even when an insect has a target, it may occasionally make a random move, leading to more realistic and varied pollination patterns. If no flowers are nearby, they revert to random wandering.
...
-   **Eggs** (`🥚`): The offspring of insects. They remain stationary and have a fixed `hatchTimer`. Each tick, the timer decrements. When it reaches zero, the egg is removed, and a new insect of the parent's species spawns in its place. However, if a bird is on the same cell at the moment of hatching, the egg is consumed, and no insect emerges, adding another layer to the predator-prey dynamic.
-   **Birds** (`🐦`): The predators of the garden.
    -   **AI & Movement**: Birds are intelligent hunters. They use a Quadtree to efficiently scan for prey within their vision range. When not actively hunting, they exhibit a **smarter patrolling AI**, picking a flower as a temporary destination. This mimics a more natural search for prey near food sources, increasing the chances of predator-prey interactions.
